...
int8_t PN532::writeCommand(const uint8_t *header, uint8_t hlen,
...
uint8_t PN532::readGPIO(void) {
  ...
  */
  ...
  Serial.print("P3 GPIO: ");
  Serial.print(pn532_packetbuffer[7]);
  Serial.print("P7 GPIO: ");
  Serial.print(pn532_packetbuffer[8]);
  Serial.print("I0I1 GPIO: ");
...
bool PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid,
  ...
  // read data packet
  if (readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer), timeout) <
      0) {
    return 0x0;
  }
  ...
  */
  ...
  Serial.print("SAK: 0x");
  Serial.print(pn532_packetbuffer[4]);
  Serial.print("\n");
...
bool PN532::inListPassiveTarget() {
  ...
  int16_t status =
      readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer), 30000);
  if (status < 0) {
    return false;
  }
...
int8_t PN532::tgInitAsTarget(const uint8_t *command, const uint8_t len,
  ...
  status =
      readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer), timeout);
  if (status > 0) {
    return 1;
...
int8_t PN532::felica_SendCommand(const uint8_t *command, uint8_t commandlength,
  ...
  // Wait card response
  int16_t status =
      readResponse(pn532_packetbuffer, sizeof(pn532_packetbuffer), 200);
  if (status < 0) {
    Serial.print("Could not receive response\n");
    return -3;
  }
...
int8_t PN532::felica_RequestSystemCode(uint8_t *numSystemCode,
  ...
  *numSystemCode = response[9];
  ...
  if (responseLength < 10 + 2 * *numSystemCode) {
    Serial.print(
        "Request System Code command failed (wrong response length)\n");
    return -2;
  }
