...
// ISO7816-4 commands
#define ISO7816_SELECT_FILE 0xA4   // 164
#define ISO7816_READ_BINARY 0xB0   // 176
#define ISO7816_UPDATE_BINARY 0xD6 // 214

// 164 164 176 164 176 176
...
void EmulateTag::setNdefFile(const uint8_t *ndef, const int16_t ndefLength) {
  if (ndefLength > (NDEF_MAX_LENGTH - 2)) {
    Serial.print("ndef file too large (> NDEF_MAX_LENGHT -2) - aborting");
    return;
  }
...
void EmulateTag::setUid(uint8_t *uid) { uidPtr = uid; }
...
bool EmulateTag::emulate(const uint16_t tgInitAsTargetTimeout) {
  ...
  Serial.print("tgInitAsTarget: ");
  if (1 !=
      pn532.tgInitAsTarget(command, sizeof(command), tgInitAsTargetTimeout)) {
    Serial.print("tgInitAsTarget failed or timed out!");
    return false;
  }
  Serial.print("tgInitAsTarget success!\n");
  ...
  uint8_t compatibility_container[] = {
      ...
      0x06, // L
      0xE1,
      0x04, // File identifier
      ((NDEF_MAX_LENGTH & 0xFF00) >> 8),
      (NDEF_MAX_LENGTH & 0xFF), // maximum NDEF file size
  ...
  while (runLoop) {
    ...
    case ISO7816_SELECT_FILE:
      Serial.println("ISO7816_SELECT_FILE");
      switch (p1) {
      case C_APDU_P1_SELECT_BY_ID:
        Serial.println("C_APDU_P1_SELECT_BY_ID");
        if (p2 != 0x0c) {
          Serial.print("C_APDU_P2 != 0x0c\n");
          setResponse(COMMAND_COMPLETE, rwbuf, &sendlen);
        } else if (lc == 2 && rwbuf[C_APDU_DATA] == 0xE1 &&
                   (rwbuf[C_APDU_DATA + 1] == 0x03 ||
      ...
      case C_APDU_P1_SELECT_BY_NAME:
        Serial.println("C_APDU_P1_SELECT_BY_NAME");
        const uint8_t ndef_tag_application_name_v2[] = {
            0, 0x7, 0xD2, 0x76, 0x00, 0x00, 0x85, 0x01, 0x01};
        if (0 == memcmp(ndef_tag_application_name_v2, rwbuf + C_APDU_P2,
                        sizeof(ndef_tag_application_name_v2))) {
          setResponse(COMMAND_COMPLETE, rwbuf, &sendlen);
        } else {
          Serial.print("function not supported\n");
    ...
    case ISO7816_READ_BINARY:
      Serial.println("ISO7816_READ_BINARY");
      switch (currentFile) {
      case NONE:
        setResponse(TAG_NOT_FOUND, rwbuf, &sendlen);
        break;
    ...
    case ISO7816_UPDATE_BINARY:
      Serial.println("ISO7816_UPDATE_BINARY");
      if (!tagWriteable) {
        setResponse(FUNCTION_NOT_SUPPORTED, rwbuf, &sendlen);
      } else {
        if (p1p2_length > NDEF_MAX_LENGTH) {
...
void EmulateTag::setResponse(responseCommand cmd, uint8_t *buf,
                             uint8_t *sendlen, uint8_t sendlenOffset) {
  switch (cmd) {
