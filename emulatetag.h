...
#define NDEF_MAX_LENGTH                                                        \
  128 // altough ndef can handle up to 0xfffe in size, arduino cannot.
typedef enum {
  COMMAND_COMPLETE,
  TAG_NOT_FOUND,
...
public:
  ...
  /*
   * @param uid pointer to byte array of length 3 (uid is 4 bytes - first byte
   * is fixed) or zero for uid
   */
  ...
  bool writeOccured() { return tagWrittenByInitiator; }

  void setTagWriteable(bool setWriteable) { tagWriteable = setWriteable; }

  uint8_t *getNdefFilePtr() { return ndef_file; }

  uint8_t getNdefMaxLength() { return NDEF_MAX_LENGTH; }
